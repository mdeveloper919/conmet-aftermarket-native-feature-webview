  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (app.isFetching) {
      ...
      <View style={styles.main.containerWithNav} >
        <Text allowFontScaling={false} style={styles.main.subHead}>Choose the Hub Assembly Type</Text>
          {app.filterResults[FILTERIDX].map((result, index) => {
						return (<GeneralButton
							style={styles.buttons.general}
							onPress={()=> { setFilter(FILTERIDX, {hatyp: result.Id}, app)}}
...
export default connect(mapStateToProps, mapDispatchToProps)(HubType);
